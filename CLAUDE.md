...
- `models/` holds the vendored ONNX model files the similarity route loads at runtime (`env.allowRemoteModels = false`), so cold starts never download from Hugging Face. `next.config.js` traces them into the function. If the model changes, replace these files too.
- The homepage is statically prerendered at build time; data changes go live on deploy.
